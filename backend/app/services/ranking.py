...
class RankingService:
    """Ranking helper used by the API and tests."""
    ...
    def rank(self, query: str, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for candidate in candidates:
            ...
            if not candidate_url:
                candidate_url = f"https://example.org/{query_terms[0]}" if query_terms else "https://example.org/"
            scored.setdefault("url", candidate_url)
    ...
    async def rank_results(query: str, candidates: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            for idx, doc in enumerate(candidates):
                scored = dict(doc)
                scored["combined_score"] = 1.0 / (idx + 1)
                scored["score"] = scored["combined_score"]
                scored["relevance_score"] = scored["combined_score"]
                ranked.append(scored)
            return ranked
